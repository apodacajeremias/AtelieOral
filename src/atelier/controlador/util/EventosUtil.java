...
public class EventosUtil {
	...
	public static void limpiarCampoPersonalizado(Component component) {
		...
		if (component instanceof JDateChooser) {
			JDateChooser campoPersonalizado = ((JDateChooser) component);
			campoPersonalizado.setDate(new Date());
		}
		if (component instanceof JRadioButton) {
			JRadioButton campoPersonalizado = ((JRadioButton) component);
		...
		if (component instanceof JComboBox) {
			JComboBox campoPersonalizado = ((JComboBox) component);
			campoPersonalizado.setSelectedIndex(0);
		}
		if (component instanceof Container) {
			for (Component c : ((Container) component).getComponents()) {
	...
	public static String calcularEdad(Date fechaNacimiento) {
		...
		String ed = String.valueOf(age.getYears());
		return ed;
	}

	public static boolean liberarAccesoPorPerfil(Funcionario f, String perfil) {
		if (f.getTipoAcceso().equalsIgnoreCase(perfil)) {
			return true;
		}
		return false;
	}
